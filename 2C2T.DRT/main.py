...
def load_or_generate_data():
    ...
    def read_images(path, num=None):
        with gzip.open(path, 'rb') as f:
            ...
            return np.frombuffer(raw, dtype=np.uint8).reshape(n, 1, rows, cols).astype(np.float32) / 255.0
...
def build_model(model_type="mlp"):
    if model_type == "mlp":
        return c2t.Sequential(
            c2t.Flatten(),
            c2t.Dense(784, 512), c2t.ReLU(), c2t.Dropout(0.3),
            c2t.Dense(512, 256), c2t.ReLU(), c2t.Dropout(0.2),
            c2t.Dense(256, 128), c2t.ReLU(),
            c2t.Dense(128, 10),
        )
    elif model_type == "large":
        return c2t.Sequential(
            c2t.Flatten(),
            c2t.Dense(784, 2048), c2t.ReLU(), c2t.Dropout(0.3),
            c2t.Dense(2048, 2048), c2t.ReLU(), c2t.Dropout(0.3),
            c2t.Dense(2048, 1024), c2t.ReLU(), c2t.Dropout(0.2),
            c2t.Dense(1024, 512), c2t.ReLU(),
            c2t.Dense(512, 10),
        )
        ...
        for i in range(len(dims) - 2):
            layers.append(c2t.Dense(dims[i], dims[i+1]))
            layers.append(c2t.ReLU())
        ...
        return c2t.Sequential(
            c2t.Conv2D(1, 32, kernel_size=3, padding=1), c2t.ReLU(),
            c2t.Conv2D(32, 64, kernel_size=3, padding=1), c2t.ReLU(),
            c2t.Flatten(),
            c2t.Dense(64 * 28 * 28, 256), c2t.ReLU(), c2t.Dropout(0.3),
            c2t.Dense(256, 10),
        )
    else:
...
def main():
    ...
    parser.add_argument("--model", choices=["mlp", "large", "deep", "huge", "cnn"], default="mlp")
    ...
    scheduler = c2t.LRScheduler(optimizer, factor=0.5, patience=3, min_lr=1e-6, verbose=True)
    ...
    if history["val_loss"]:
        ...
        print(f"  Best Val Loss: {history['val_loss'][best_val_idx]:.4f} "
